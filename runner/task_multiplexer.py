...
def preprocess_data(task_id, task_tag, output_dir, algo_list, df):
    logging.info("Preprocessing data for task %s with tag %s and algorithms %s", task_id, task_tag, algo_list)
    # breakpoint()
    ...
    logging.info("applying stopwords")
    df["Narrative"] = df["Narrative"].apply(get_stopword_func(algo_list))
    data_tools.save_csv_file(df, 'x_df_stopwords.csv', output_dir=output_dir)

    # TODO: fork for snowball_stemmer
    logging.info("applying stemmer")
    df["Narrative"] = df["Narrative"].apply(get_stemmer_func(algo_list))
    data_tools.save_csv_file(df, 'x_df_stemmer.csv', output_dir=output_dir)

    logging.info("Preprocessed DataFrame head: %s", df.head())
    return df


def split_data(x_df, y_df, k_df, split_date, seed, output_dir):
    logging.info("Splitting data into train and test sets with split date %s and seed %d", split_date, seed)
    (xtr_df, ytr_df, ktr_df), (xtest_df, ytest_df, ktest_df) = data_tools.split_into_train_test(
        xdf=x_df,
        ydf=y_df,
        kdf=k_df,
...
def train_classifier(xtr_df, ytr_df, ktr_df, xte_df, yte_df, kte_df, algo_list, output_dir, seed):
...
def get_stopword_func(algo_list):
    if 'sw.csw' in algo_list:
        ...
        raise ValueError("No valid stopword function found in the algorithm list: %s" % algo_list)

def get_vectorizer_func(algo_list):
    if 'vec.tfidf' in algo_list:
        logging.info("Using TF-IDF vectorizer")
        return data_tools.tfidf_vectorizer
    elif 'vec.bow' in algo_list:
        logging.info("Using Bag of Words vectorizer")
        return data_tools.count_vectorizer
    else:
        raise ValueError("No valid vectorizer function found in the algorithm list: %s" % algo_list)
...
def get_stemmer_func(algo_list):
    if 'stem.snow' in algo_list:
        ...
        raise ValueError("No valid stemmer function found in the algorithm list: %s" % algo_list)
...
def get_classifier_func(algo_list):
    if "clf.nb" in algo_list:
        ...
        raise ValueError("No valid classifier function found in the algorithm list: %s" % algo_list)
